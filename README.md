...
- **Tracking lives in Linear:** one team, **`Mikrosag` (key `MIK`)**, covering all five `mikrosag` repos. Workflow states are unchanged from the old convention: `Backlog → Todo → In Progress → In Review → Done`, plus Linear's default `Cancelled`. One team rather than one per repo — five teams is admin overhead with no benefit at this scale, and Linear's free tier caps at 2 teams / 250 issues, so a single team leaves headroom.
- **Repo scoping is a label, not a link:** every issue carries exactly one repo label — `homelab`, `automation`, `ci-common`, `mac-config`, `mcp-gateway`. This is deliberately what fixes the old setup's coverage gap: the `mikrosag Roadmap` GitHub Project was linked to only `automation` and `ci-common`, because Projects v2 items are repo-linked and adding a repo was fiddly enough that it never happened. Labels aren't repo-linked, so there is no reason to leave a repo out.
