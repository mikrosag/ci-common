...
- **Working a kanban card?** Pick it up from the [`mikrosag Roadmap`](https://github.com/orgs/mikrosag/projects/1) GitHub Project, not just a bare issue. When you start, move its Status to `In Progress`. When your PR merges, move it to `Done` *and* update the matching row in the Notion `mikrosag Roadmap` database (same Status, `GitHub URL` set to the issue/PR link) — there's no automatic sync, this is a manual step every agent does as part of closing out the card. See `README.md`'s Kanban section for the full convention.
